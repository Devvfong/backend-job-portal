...
const suspendCompanyController = async (req, res, next) => {
  try {
    ...
    if (suspend) {
      await notifyCompany(updated.id, buildCompanyAlertNotification(updated, "suspended", reasonArray));
    }
...
const warnCompanyController = async (req, res, next) => {
  try {
    ...
    await notifyCompany(updated.id, buildCompanyAlertNotification(updated, "warning", reasonArray));
